...
def get_url(**kwargs):
    """Build a plugin URL with URL-encoded values so '&' in the stream URL
    does not break the query string."""
    parts = []
...
def list_channels():
    ...
    for ch in channels:
        ...
        # Pass channel_id instead of pre-built stream_url (prevents expired JWT issues)
...
def router(paramstring):
    ...
    if params.get("action") == "play":
        ...
        # Build a FRESH stream URL at the moment of playback
        client = PlutoClient()
        try:
