...
Mirrors the same calls Pluto's own web player (pluto.tv) makes:
  1. GET boot.pluto.tv/v4/start      -> session token + ad-stitch params
  2. GET service-channels.clusters.pluto.tv/v2/guide/channels -> channel list
  3. Stream URL for each channel is built from the channel id via
...
for anonymous/guest viewing. All requests use the device's real network
location (no spoofed headers), so the channel lineup and content shown
will match whatever region the device is actually in -- same as if you
opened pluto.tv in a browser on this device.
"""
